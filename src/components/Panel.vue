...
const resize = () => {
  ...
  if (state === 0) {
    ...
    handle.value.style.left = `${s - handleWidth() / 2 / 2}px`
...
function updateSize(_s: number | { 1: number } | { 2: number }) {
  ...
  if (state === 0) {
    ...
    size = Math.min(Math.max(s, low), high)
    ...
    size = { 2: Math.min(Math.max(s, low), high) }
...
<template>
  <div ref="container" class="panel-container">
    <div ref="d1" class="panel">
      <slot name="1"></slot>
    </div>
    <div ref="d2" class="panel">
      <slot name="2"></slot>
    </div>
    <div ref="handle" class="handle" @mousedown="onMousedown"></div>
  </div>
</template>
